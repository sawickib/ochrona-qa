...
\begin{solution}
Kopie zapasowe (backup) służą do odtworzenia danych w przypadku ich utraty lub uszkodzenia i zazwyczaj są regularnie tworzone dla aktualnych danych. System archiwizacji polega na przechowywaniu danych nieaktywnych lub rzadko używanych w celu długoterminowego zachowania informacji, przy czym archiwa nie są zwykle gotowe do odtworzenia systemu w razie awarii​. Jego zadaniem jest utrwalenie historii danych.
\end{solution}
...
\item[DMZ]
DMZ (ang. Demilitarized Zone) to wydzielony obszar w sieci komputerowej, oddzielony od sieci wewnętrznej i zewnętrznej (np. Internetu) za pomocą zapór sieciowych. W DMZ umieszczane są serwery świadczące usługi zarówno dla użytkowników z sieci zewnętrznej, jak i wewnętrznej, takie jak serwery WWW czy pocztowe, aby zwiększyć bezpieczeństwo głównej sieci wewnętrznej, ograniczając dostęp do niej​.
\item[Firewall]
...
\item[Honeypot] - Honeypot to celowo wystawiony system lub zasób informatyczny, zaprojektowany w taki sposób, aby przyciągać potencjalnych atakujących. Jego głównym celem jest monitorowanie, analiza technik ataku oraz gromadzenie informacji o działaniach intruzów, co pozwala na lepsze zrozumienie zagrożeń i opracowanie skuteczniejszych mechanizmów obrony. Naruszenie bezpieczeństwa Honeypot nie wpływa na bezpieczeństwo właściwego systemu.
\item[IPS] - Intrusion Prevention System to system ochrony sieci, który monitoruje ruch w czasie rzeczywistym, wykrywając i blokując potencjalnie złośliwe działania. Działa prewencyjnie, zapobiegając dotarciu zagrożeń do chronionych zasobów.
\item[DLP]
...
\begin{solution}
Ewidencja i klasyfikacja zasobów informatycznych umożliwiają dokładne określenie, jakie zasoby są w posiadaniu organizacji, oraz ich wartości i krytyczności. Dzięki temu można odpowiednio priorytetyzować działania ochronne, stosować właściwe środki zabezpieczeń oraz szybko reagować na zagrożenia, co wzmacnia skuteczność całego systemu ochrony​.
\end{solution}

\question Na czym polega audyt polityki bezpieczeństwa?
\begin{solution}
Audyt polityki bezpieczeństwa polega na weryfikacji, czy dokumentacja polityki bezpieczeństwa organizacji oraz jej wdrożenie są zgodne z przyjętymi standardami i rzeczywistym stanem systemów ochrony. Obejmuje identyfikację braków, nieścisłości oraz ocenę skuteczności stosowanych zabezpieczeń, a także rekomendacje dotyczące ich poprawy i dostosowania do zmieniających się warunków działalności instytucji.
\end{solution}
...
\begin{solution}
Norma PN-ISO 17799 (obecnie zastąpiona przez PN-ISO/IEC 27002) dotyczy wytycznych związanych z zarządzaniem bezpieczeństwem informacji w organizacjach. Obejmowała rekomendacje dotyczące ustanawiania, wdrażania, utrzymania i doskonalenia polityki bezpieczeństwa informacji, w tym ochrony poufności, integralności i dostępności danych​.
\end{solution}

\question Jakie dane chronione są przez ustawę o ochronie danych osobowych?
\begin{solution}
Ustawa o ochronie danych osobowych chroni dane umożliwiające zidentyfikowanie osoby fizycznej, takie jak imię, nazwisko, adres, numer PESEL, dane biometryczne czy genetyczne. Szczególną ochroną objęte są dane wrażliwe, m.in. dotyczące zdrowia, poglądów politycznych, przekonań religijnych, życia seksualnego czy przynależności wyznaniowej lub związkowej​.
\end{solution}
