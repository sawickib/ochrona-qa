...
\item Serewer losuje $r$ i odsyła klientowi.
...
\begin{solution}
'Security-by-separation' to model, w którym ograniczamy skutki błędów do tylko jednego modułu. Możemy to uzyskać dzięki zabezpieczaniu każdego modułu niezależnie od innych, z negatywnym nastawieniem do reszty programu.
\end{solution}
...
\begin{solution}
Kopie zapasowe mogą zapewniać wysoką dostępność danych, jeśli są odpowiednio przechowywane. Możliwe są następujące sposoby przechowywania, zabezpieczania kopii:
\begin{itemize}
\item on-line (np. dodatkowy dysk) - zapewnia szybki dostęp, lecz dane są podatne na ataki i awarie,
\item off-line (np. płyty DVD) - dane odporne na ataki i awarie, lecz dostęp do nich jest wolny,
\item zewnętrze, bezpieczne miejsce - dane są odporne na katastroficzne zniszczenia.
\end{itemize}
\end{solution}
