...
\end{itemize}
Nie trudno zauważyć, że OTP jest mało praktyczny w zwykłych zastosowaniach. Używa się go do zastosowań specjalnych, a także jego elementy można znaleźć innych algorytmach.
\end{solution}
...
H(x)= - \sum_{i=1}^n p(i) \ log_2 p(i)
\end{equation}
Obliczamy prawdopodobieństwo wystąpienia w ciągu każdej z liter \textit{P = liczba wystąpień danej litery $/$ długość całego ciągu}
np. $ P(b) = 4/8 = 0.5 $ itp. Podstawiamy wartości do wzoru ogólnego i otrzymujemy entropię.
\begin{equation}
...
\begin{solution}
Bezpieczne generatory liczb losowych (eng. CSPRNG) stanowią szczególną klasę generatorów pseudolosowych. Aby generator mógł być uznany za CSPRNG musi przejść \textit{test następnego bitu} - nie może istnieć wielomianowy algorytm, który potrafi przewidzieć kolejny generowany bit z prawdopodobieństwem znacząco większym niż $0.5$. Otrzymuje się to w ten sposób, że przy tworzeniu ziarna generator korzysta z odpowiedniego źródła danych - odpowiedniego tzn. jak najmniej przewidywalnego, np. ,,machanie'' myszką po ekranie przez określony czas, zmiany temperatury procesora/płyty głównej, parametry pracy dysku. Używane są głównie do inicjowania kluczy lub sesji. Opierają się na algorytmach kryptograficznych (DES, RSA, MD5, SHA) lub są zaprojektowane specjalne do takich celów (Yarrow, Blum Blum Shub).
\end{solution}

%wtq
\question Wyjaśnij w jaki sposób komputer będący deterministyczną maszyną stanów może dokonać aktu losowania.
\begin{solution}
Komputer sam z siebie nigdy nie wygeneruje niczego losowego. Każdy algorytm jest funkcją przekształcającą bieżący stan komputera w następny (przekształca zbiór skończony w siebie). Każda taka funkcja po pewnej liczbie iteracji wpadnie w cykl, a co za tym idzie nigdy nie uzyskamy losowości. Komputer jest w stanie wytworzyć coś, co wygląda jak losowe (patrz wyżej - generatory liczb pseudolosowych). Jednak komputer to też dyski, urządzenia wejścia/wyjścia, szumy i zakłócenia. To właśnie szumów i zakłóceń możemy użyć do wykonania aktu losowania.
\end{solution}
...
\begin{solution}
Funkcja silnie bezkonfliktowa musi spełniać warunek, który mówi, że praktycznie niemożliwe jest wyznaczenie elementów $x$ i $x'$ takich, że $H(x) = H(x')$.
\end{solution}
...
\begin{solution}
W kryptografii asymetrycznej używane są dwa rodzaje kluczy - klucz publiczny i klucz prywatny. Klucz publiczny użytkownika jest powszechnie znany, natomiast do klucza prywatnych dostęp ma tylko jego właściciel. Wynika z tego, że kluczem publicznym zaszyfrowujemy informacje, a klucz prywatny służy do jej odczytu. Prosty schemat:
\begin{enumerate}
\item Kasia chce wysłać zaszyfrowaną wiadomość do Janka. Klucz publiczny Janka jest ogólnodostępny (np. umieszczony w jego podpisie na forum dyskusyjnym). Kasia szyfruje swoją wiadomość kluczem publicznym Janka i wysyła.
\item Janek otrzymuje zaszyfrowaną wiadomość. Używa swojego klucza prywatnego do jej odszyfrowania.
\end{enumerate}
Prościej:
\begin{enumerate}
\item Janek przesyła Kasi swój klucz publiczny.
\item Kasia podpisuje kluczem publicznym Janka wiadomość dla niego.
\item Janek odszyfrowuje wiadomość swoim kluczem prywatnym.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item Kasia jest stroną uwierzytelniającą (chcącą potwierdzić swoją tożsamość). Wylicza hash podpisywanej wiadomości. Szyfruje hash swoim kluczem prywatnym i jako podpis dołącza do oryginalnej wiadomości.
\item Dowolna osoba posiadająca klucz publiczny Kasi może sprawdzić autentyczność podpisu, poprzez odszyfrowanie hashu za pomocą klucza publicznego Kasi i porównanie go z własnoręcznie wyliczonym na podstawie oryginalnej wiadomości.
\end{enumerate}
Prościej:
\begin{enumerate}
\item Kasia przesyła Jankowi swój klucz publiczny.
\item Kasia podpisuje kluczem prywatnym wiadomość dla Janka.
\item Janek sprawdza czy wiadomość pochodzi od Kasi jej kluczem publicznym.
\end{enumerate}
\end{solution}
...
\begin{solution}
Test pierwszości to algorytm określający, czy dana liczba jest pierwsza, czy złożona. Probalistyczny test pierwszości przebiega w następujący sposób:
\begin{enumerate}
...
\begin{solution}
Klucze publiczne w protokole SSH umieszczone są w odpowiednim miejscu na serwerze, do którego użytkownik chce się połączyć.
\end{solution}
...
\begin{solution}
Uwierzytelniane dwuskładnikowe polega na połączeniu dwóch metod uwierzytelniania, najczęściej są to hasło i tokeny sprzętowe.
\begin{itemize}
\item większość banków internetowych stosuje hasło + token, jako aplikacja mobilna (a użycie samego tokenu wymaga podania dodatkowego hasła, np. w postaci kodu PIN)
\item hasło + hasło jednorazowe przesyłane za pomocą SMS
\item hasło + rozmowa telefoniczna, w której otrzymamy dodatkowy PIN
\end{itemize}
\end{solution}
...
\begin{solution}
Dowody z wiedzą zerową to takie procedury kryptograficzne, za pomocą których udowadniamy fakt posiadania pewnej informacji, bez ujawniania jej. Innymi słowy: wykazujemy prawdziwość pewnego stwierdzenia bez ujawniania niczego poza tą prawdziwością.
\end{solution}
\end{questions}
...
\begin{solution}
Schemat składa się z dwóch podstawowych etapów: \textit{generowania podpisu cyfrowego} i \textit{weryfikacji podpisu cyfrowego}.
Generowanie:
\begin{enumerate}
\item Strona uwierzytelniająca wylicza hash podpisywanej wiadomości.
\item Hash jest szyfrowany kluczem prywatnym użytkownika
...
\begin{solution}
Funkcja haszująca stosowana jest do wyliczenia hasha podpisywanej wiadomości. Hash ten jest szyfrowany kluczem prywatnym i jako podpis cyfrowy dołączany do oryginalnej wiadomości. Jeżeli w trakcie przesyłania, wiadomość wpadnie w niepowołane ręce i zostanie w niej coś zmienione (np. w umowie na 20 stron dopisane drobnym druczkiem zobowiązanie do wpłacenia 100000zł) to po porównaniu odszyfrowanego hasha i własnoręcznie wykonanego na wersji zmienionej - będzie o tym wiadomo. Wtedy można poprosić drugą stronę o ponowne przesłanie wiadomości inną drogą. Reasumując, użycie funkcji haszującej jest konieczne do tego, by można było mówić o podpisie cyfrowym. Zaszyfrowana wartość hash to właśnie podpis cyfrowy.
\end{solution}
...
\begin{solution}
Wzajemne uwierzytelnianie, które służy na przykład do wykrywania ataku man-in-the-middle. Podstawową zasadą jest to, że nie deszyfruje się klucza mając tylko połowę kryptogramu(0.5 krypto$=>$ 0.5 tekstu jawnego).
\begin{enumerate}
...
Już reguła Kerkhoffa odradzała stosowania takiej techniki: ,,Nie opieramy sie na tajności algorytmu, a na pewności klucza.'' Podkreśla to fakt, iż bezpieczeństwo nie tkwi w tajności budowy algorytmu, ale w jego konstrukcji.
\end{solution}
...
%Czy chodzi tutaj o MAC (Model kontroli obowiązkowej) zwany również systemem zaufanym?
\question Na czym polega model wielopoziomowej ochrony? Jakie są jego zalety?
...
\begin{solution}
\begin{itemize}
\item walidacja poprawności wprowadzonych danych po stronie serwera,
\item użycie funkcji filtrujących wybrane znaki pól tekstowych,
\item zastosowanie zapytań parametryzowanych,
\item ograniczenie uprawnień użytkowników do jak najmniejszych,
\item nieinformowanie użytkownika o szczegółach błędów występujących w bazie po wpisaniu niepoprawnego zapytania.
\end{itemize}
\end{solution}

%wtq
\question Omów przykład błędu skutkujący możliwością SQL-Injection?
\begin{solution}
\textit{SELECT 1 FROM login WHERE login = 'admin' AND pw = 'haslo'} \\
Powyższe zapytanie odpowiada sytuacji, kiedy użytkownik logując się w formularzu logowania wpisał $admin$ jako użytkownik, natomiast jako hasło $haslo$. Działanie tego zapytania polega na tym, że zwróci ono jeden wiersz tylko wtedy, gdy warunek po WHERE będzie spełniony. W przeciwnym wypadku nie zostanie zwrócony żaden wiersz. Problem w tym przypadku polega na tym, że zarówno login i hasło są wpisywane bezpośrednio do zapytania, więc użytkownik logując się, modyfikuje to zapytanie.
\end{solution}
...
\begin{solution}
\begin{itemize}
\item sprawdzać surowe nagłówki maili; zwracać uwagę przez jakie serwery przeszedł mail, ponieważ w sieci można znaleźć pełno serwisów, w których podszyjemy się pod określony adres mailowy, np. $emkei.cz$
\item sprawdzać certyfikaty; nie wystarczy spojrzeć na kłódkę przy adresie - jeżeli strona wydaje się podejrzana to warto spojrzeć na szczegóły certyfikatu,
\item używać zawsze najnowszych wersji oprogramowania (najnowsze wersje przeglądarek wyposażone są w filtry antyphishingowe),
\item używać OpenDNS,
\item nie otwierać linków prosto z maila,
\item nie przesyłać mailem żadnych danych typu hasła, numery kart kredytowych; wszystkie takie prośby należy ignorować.
\end{itemize}
\end{solution}
...
\question Omów funkcjonalności związane z kopiami zapasowymi.

%wtq
\question Wyjaśnij czym różni się kopia zapasowa pełna, przyrostowa i różnicowa.
\begin{solution}
\begin{description}
\item[Kopia pełna] zawsze musimy wykonywać ją od początku. Tworzymy pełny obraz archiwizowanych plików bez względu na to, czy od wykonania ostatniego obrazu w plikach zaszły jakieś zmiany. Największy rozmiar i najdłuższy czas wykonywania.
